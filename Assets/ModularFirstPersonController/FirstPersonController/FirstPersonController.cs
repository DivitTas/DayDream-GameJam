    ...
    void Start()
        ...
        private void WallRunning()
        {
            ...
            rb.AddForce(wallForward * 3f, ForceMode.Force);
        ...
        private bool CheckForWall()
        {
            if (orientation == null)
            {
                Debug.LogWarning("FirstPersonController: orientation not assigned — defaulting to transform.");
                orientation = transform;
            }
            wallRight = Physics.Raycast(transform.position+Vector3.up * 0.5f, transform.right, out rightWallHit, wallCheckDistance, wallLayer);
            wallLeft = Physics.Raycast(transform.position+ Vector3.up*0.5f, -transform.right,out leftWallHit, wallCheckDistance, wallLayer);
            Debug.Log(wallLeft);
            Debug.Log(wallRight);
